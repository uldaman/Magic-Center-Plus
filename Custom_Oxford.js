...
class Custom_Oxford {
  ...
  async displayName() {
    ...
    if (locale.indexOf("CN") != -1) return "牛津英汉双解(改)";
    if (locale.indexOf("TW") != -1) return "牛津英汉双解(改)";
  ...
  async findOxford(word) {
    ...
    function buildDefinitionBlock(exp, pos, defs) {
      ...
      for (const def of defs) {
        ...
        if (def.tag == "id" || def.tag == "pv") definition += def.enText ? `<div class="idmphrase">${def.enText}</div>` : "";
        //if (def.tag == 'xrs')
        //    definition += `<span class='tran'><span class='eng_tran'>${def.data[0].data[0].text}</span></span>`;
        if (def.tag == "d" || def.tag == "ud")
          definition += pos + `<span class='tran'><span class='eng_tran'>${def.enText}</span><span class='chn_tran'>${def.chText}</span></span><br>`;
    ...
    function getOxford(data) {
      try {
        ...
        for (const entry of entries) {
          if (entry.tag == "p-g" || entry.tag == "h-g") {
            ...
            for (const group of entry.data) {
              ...
              if (group.tag == "d") {
                definition += pos + `<span class='tran'><span class='eng_tran'>${group.enText}</span><span class='chn_tran'>${group.chText}</span></span><br>`;
        ...
        let css = Custom_Oxford.renderCSS();
  ...
  static renderCSS() {
    let css = `
            <style>
                ...
                span.pos  {text-transform:lowercase; font-size:0.9em; margin-right:5px; padding:0.8px 4px; color:white; background-color:#0d47a1; border-radius:3px;}
                span.tran {margin:0; padding:0;}
                span.eng_tran {margin-right:3px; padding:0;}
                span.chn_tran {color:#0d47a1;font-size:13px;}
                ...
                span.chn_sent {color:#0d47a1;font-size:13px;}
            </style>`;
    return css;
  }
}
